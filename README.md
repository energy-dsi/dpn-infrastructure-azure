...
| `ampls` | Optional: forces Log Analytics ingestion/query traffic through a private endpoint instead of the public Azure Monitor endpoint. Not wired into the root config by default - see the module README before enabling it. |
| `bastion` + `vm` | The admin access path: a Windows jump host (`vm`) reachable only through Azure Bastion (`bastion`), used by platform engineers to reach the private AKS API server and other private-endpoint-only resources for day-to-day operations. |
...
**No license is currently attached to this repository.** OpenTofu, Terraform, Bicep, and the CI/CD platforms this codebase runs on are all open source - but that does not make this codebase open source, and using open-source tooling grants no rights over the configuration written for it. Under standard copyright law, original content is "all rights reserved" by default the moment it's created, with or without a public repository, regardless of what license the underlying tools use. Until a LICENSE file is added, nobody outside the original author/organization has an explicit legal right to use, modify, or redistribute this codebase - if you're distributing it to other customers, resolve this deliberately rather than by default. Not legal advice - confirm with whoever handles legal/IP at your organization before wide distribution.
